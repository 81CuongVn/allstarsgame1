<?php
	class PlayerWanted extends Relation {
		static function filter($page, $limit) {
			...
			$result['players']	= PlayerWanted::find('death=0', ['limit' => ($page * $limit) . ', ' . $limit, 'reorder' => 'player_id ASC']);
		...
		function embed(){
            ...
            $iv = substr(CHAT_SECRET, 0, 16);
            return openssl_encrypt(json_encode($embed), 'AES-256-CBC', CHAT_SECRET,0, $iv);
		}
		function chat_embed() {
			return '';
		}
