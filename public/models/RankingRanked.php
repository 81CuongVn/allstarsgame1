<?php
class RankingRanked extends Relation {
	function graduation() {
	...
	static function filter($where,$where2, $page, $limit) {
		...
		if(!$where && !$where3) {
			...
			$result['pages']	= ceil(Recordset::query('SELECT COUNT(id) AS _max FROM (SELECT id FROM ranking_rankeds WHERE 1=1 ' . $where . ') _w')->row()->_max / $limit);
			$result['players']	= RankingRanked::find('1=1 ' . $where .' '. $where3, ['limit' => ($page * $limit) . ', ' . $limit, 'reorder' => ''.$where2.' ASC']);
		}
