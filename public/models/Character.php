<?php
	class Character extends Relation {
		...
		function consumables($shop = false) {
		    $addSql = '';
		    if ($shop)
		        $addSql = 'AND id NOT IN (1859,1863,2102,2103,2058,2059)';

			$result		= [];
			$items		= Item::find('item_type_id = 5 ' . $addSql . ' ORDER BY price_currency ASC', array('cache' => true));
		...
		function filter($where,$mine_pets,$player,$active, $page, $limit) {
			...
			$result['pets']	= Recordset::query('
				...
				WHERE
					language_id=' . $_SESSION['language_id'] . ' AND b.item_type_id=3 '. $where . $where2 ." LIMIT ".$page * $limit.",". $limit);
