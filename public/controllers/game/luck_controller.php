...
class LuckController extends Controller {
	...
	private	$summon_currency	= 10000;
	private	$summon_credits		= 5;
	...
	public function summon() {
		...
		$this->assign('reward_list',		Recordset::query('
			...
			FROM
				luck_rewards a
				LEFT JOIN player_luck_logs b ON b.luck_reward_id = a.id AND b.player_id = ' . $player->id . '
			WHERE a.type = 2
			GROUP BY a.id
		'));
	}
	...
	public function summoning() {
		...
		if (!sizeof($errors)) {
			...
			$this->json->message	= t('luck.index.won2', [ 'prize' => $message ]);
			$this->json->currency	= $player->currency;
			$this->json->currency	= $player->exp;
			$this->json->credits	= $user->credits;
		} else {
