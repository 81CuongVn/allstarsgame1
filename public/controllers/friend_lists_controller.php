	...
	function gift(){
		...
		if ((!isset($_POST['gift']) || (isset($_POST['gift']) && !is_numeric($_POST['player']))) && (!isset($_POST['player']) || (isset($_POST['player']) && !is_numeric($_POST['player'])))) {
			...
			if(!sizeof($errors)) {
				...
				if($gift==4){
					// Dá um Tema Random
					$theme	= CharacterTheme::find_first('price_credits > 0 || price_currency > 0', ['reorder' => 'RAND()']);
				...
				if($gift==5){
					...
					if (!$player_gift->has_item($npc_pet->id)) {
						...
						$pm->to_id		= $player_gift->id;
						$pm->subject	= t('friends.f15');
						$pm->content	= t('friends.f20', ['name' => $player->name, 'mascote'=> $npc_pet->description()->name]);
						$pm->save();
					}else{
