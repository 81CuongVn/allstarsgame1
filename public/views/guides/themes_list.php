...
?>		
	...
	<?php if ($character->currency_lock): ?>
		<?php if ($player): ?>
			<a class="btn btn-primary unlock" data-method="1" data-id="<?php echo $character->id?>"><?php echo t('characters.create.unlocks_currency', ['amount' => $character->currency_lock, 'currency' => t('currencies.' . $player->character()->anime_id)]) ?></a>
		<?php else: ?>
	...
	<?php if ($character->credits_lock): ?>
		<a class="btn btn-primary unlock" data-method="2" data-id="<?php echo $character->id?>"><?php echo t('characters.create.unlocks_credits', ['amount' => $character->credits_lock]) ?></a>
	<?php endif ?>	
<?php
	}
?>
