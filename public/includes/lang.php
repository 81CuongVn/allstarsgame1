		...
		if(file_exists($path . '/../locales')) {
			...
			foreach ($files as $file) {
				...
				if (!$is_cached) {
					...
					if (!isset(Lang::$strings[$lid->id])) {
						Lang::$strings[$lid->id]	= [];
						Lang::$parsed[$lid->id]	= [];
					}

					Lang::$strings[$lid->id]      = array_merge(Lang::$strings[$lid->id], $data[$header]);
                    ...
                    $parsed                     = [];
					_locale_cb(Lang::$strings[$lid->id], $parsed);
					Lang::$parsed[$lid->id]	= array_merge(Lang::$parsed[$lid->id], $parsed);
