...
trait AttributeManager {
    ...
    function for_atk($raw = false) {
        ...
        $effects	= $this->get_parsed_effects();
        $base		= $this->character()->for_atk + $this->attributes()->sum_for_atk + (($this->attributes()->for_atk + $this->for_atk) / $this->attrRate['for_atk']);
    ...
    function for_def($raw = false) {
        ...
        $effects	= $this->get_parsed_effects();
        $base		= $this->character()->for_def + $this->attributes()->sum_for_def + (($this->attributes()->for_def + $this->for_def) / $this->attrRate['for_def']);
    ...
    function for_crit() {
        $effects	= $this->get_parsed_effects();
        $value		= $this->character()->for_crit + $this->attributes()->sum_for_crit + (($this->for_crit + $this->attributes()->for_crit) / $this->attrRate['for_crit']) + percent($effects['for_crit_percent'], $this->character()->for_crit) + $effects['for_crit'];
    ...
    function for_crit_inc() {
        ...
        return $base  + (($this->attributes()->for_inc_crit + $this->for_inc_crit) / $this->attrRate['for_crit_inc']) + $this->attributes()->sum_for_inc_crit + $effects['for_crit_inc'] + percent($effects['for_crit_inc_percent'], $base);
    }

    function for_abs() {
        $effects	= $this->get_parsed_effects();
        $value		= $this->character()->for_abs + $this->attributes()->sum_for_abs + (($this->for_abs + $this->attributes()->for_abs) / $this->attrRate['for_abs']) + percent($effects['for_abs_percent'], $this->character()->for_abs) + $effects['for_abs'];
    ...
    function for_abs_inc() {
        ...
        return $base + (($this->attributes()->for_inc_abs + $this->for_inc_abs) / $this->attrRate['for_abs_inc']) + $this->attributes()->sum_for_inc_abs + $effects['for_abs_inc'] + percent($effects['for_abs_inc_percent'], $base);
    }

    function for_prec() {
        $effects	= $this->get_parsed_effects();
        $value		= $this->character()->for_prec + $this->attributes()->sum_for_prec + (($this->for_prec + $this->attributes()->for_prec) / $this->attrRate['for_prec']) + percent($effects['for_prec_percent'], $this->character()->for_prec) + $effects['for_prec'];
    ...
    function for_init() {
        $effects	= $this->get_parsed_effects();
        $value		= $this->character()->for_init + $this->attributes()->sum_for_init + (($this->for_init + $this->attributes()->for_init) / $this->attrRate['for_init']) + percent($effects['for_init_percent'], $this->character()->for_init) + $effects['for_init'];
