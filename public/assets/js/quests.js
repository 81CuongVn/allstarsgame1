(function () {
    ...
    $('.guild_daily_quests_change').on('click', function () {
        var id = $(this).data('id');
        var quest = $(this).data('quest');
        lock_screen(true);

        $.ajax({
            url: make_url('quests#guild_daily_change'),
            type: 'post',
            data: { id: id, quest: quest },
            success: function (result) {
                if (result.success) {
                    location.href = make_url('quests#guild_daily');
                } else {
                    lock_screen(false);
                    format_error(result);
                }
    ...
    $('#guild_daily_quests_finish').on('click', function () {
        lock_screen(true);

        $.ajax({
            url: make_url('quests#guild_daily_finish'),
            success: function (result) {
                if (result.success) {
                    location.href = make_url('quests#guild_daily');
                } else {
