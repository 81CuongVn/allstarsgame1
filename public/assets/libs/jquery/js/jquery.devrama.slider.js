...
(function($){
	var DrSlider = function(element, options){
		...
		this.options = {
			...
			navigationColor: '#6aaee5',
			navigationHoverColor: '#a7cdec',
			...
			transition: 'fade', //random, slide-left, slide-right, slide-top, slide-bottom, fade, split, split3d, door, wave-left, wave-right, wave-top, wave-bottom
			showProgress: true,
			progressColor: '#6aaee5',
			pauseOnHover: true,
			onReady: undefined
		...
		_attachNavigation: function(){
			...
			if(!this.options.userCSS){
				...
				$nav_link.css({
					...
					'width' :  this.options.navigationType != 'number' ? '9px': '',
					'height' :  this.options.navigationType != 'number' ? '9px': '',
					'padding': '0.2em',
					'font-size': '12px',
					...
					'width': this.options.navigationType == 'number' ? $nav_link.innerHeight()+'px' : '9px',
