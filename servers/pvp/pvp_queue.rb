...
@queue			= @channel.queue('aasg_pvp_battle')
@match_size		= 2
@players		= {}
@queues			= {}
@db				= {
		...
		when 'enter_queue'
			...
			if @players.keys.length >= @match_size
				@players.keys.each do |key|
					...
					if current_player['level'].to_i >= player['level'].to_i - level_tol_down && current_player['level'].to_i <= player['level'].to_i + level_tol_up
						puts 'Match found'
